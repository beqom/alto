...
describe('Tabs', () => {
    ...
    describe('renderLinks', () => {
        ...
        it('should render Link element with props',() => {
            ...
            expect(getWrapper(defaultProps, {}).find('.tabs__link').at(0).prop('id')).toBeUndefined()
            expect(getWrapper(defaultProps, {id: 'id'}).find('.tabs__link').at(0).prop('id')).toBe('id__link--0')
            expect(getWrapper(defaultProps, {}).find('.tabs__link').at(0).prop('href')).toBe('url_1')
            expect(getWrapper(defaultProps, {currentUrl: 'url_1'}).find('.tabs__link').at(0).prop('className')).toBe('tabs__link tabs__link--active')
        })
    })
    ...
    describe('renderButtons', () => {
        ...
        it('should render with props',() => {
            ...
            expect(wrapper.find('.tabs__button').at(0).prop('id')).toBe('id__button--0')
            expect(wrapper.find('.tabs__button').at(0).prop('className')).toBe('tabs__button tabs__button--active')
            expect(wrapper.find('.tabs__button').at(0).prop('flat')).toBe(true)
        })

        it('should render button inner text from props',() => {
            const wrapper = getWrapper(defaultProps, {})

            expect(wrapper.find('.tabs__button').at(0).text()).toBe('title_1')
        })
