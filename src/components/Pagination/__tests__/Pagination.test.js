...
describe('Classic case: ', () => {
  ...
  it('render the page buttons', () => {
    ...
    const buttons = wrapper.find('.Pagination__page-button');
    expect(buttons.length).toBe(7);
    expect(buttons.at(1).text()).toBe('1');
    expect(buttons.at(5).text()).toBe('10');
    expect(buttons.at(2).text()).toBe('4');
    expect(buttons.at(4).text()).toBe('6');
    expect(wrapper.find('button.Pagination__page-button').length).toBe(6);
  });

  it('render current page button with the good className', () => {
    const wrapper = shallow(<Pagination max={10} current={5} onClick={jest.fn()} />);
    const btn = wrapper.find('.Pagination__page-button').at(3);
    expect(btn.text()).toBe('5');
    expect(btn.hasClass('Pagination__page-button--current')).toBe(true);
    expect(btn.type()).toBe('div');
  });

  it('render ellispsis', () => {
    const wrapper = shallow(<Pagination max={10} current={5} onClick={jest.fn()} />);
    expect(wrapper.children().at(2).hasClass('Pagination__ellipsis')).toBe(true);
    expect(wrapper.children().at(2).text()).toBe('...');
    expect(wrapper.children().at(6).hasClass('Pagination__ellipsis')).toBe(true);
    expect(wrapper.children().at(6).text()).toBe('...');
  });
});
...
describe('onClick event: ', () => {
  ...
  it('clicking on page buttons', () => {
    ...
    wrapper.find('.Pagination__page-button').at(2).simulate('click');
  ...
  it('clicking left arrow', () => {
    const onClick = jest.fn();
    const wrapper = shallow(<Pagination max={10} current={5} onClick={onClick} />);
    wrapper.find('.Pagination__page-button--arrow').first().simulate('click');
    expect(onClick).toHaveBeenCalledTimes(1);
    expect(onClick).toHaveBeenCalledWith(4);
  });
  ...
  it('clicking right arrow', () => {
    const onClick = jest.fn();
    const wrapper = shallow(<Pagination max={10} current={5} onClick={onClick} />);
    wrapper.find('.Pagination__page-button--arrow').last().simulate('click');
    expect(onClick).toHaveBeenCalledTimes(1);
...
describe('Buttons edge cases: ', () => {
  ...
  it('render current page 1 if current === 0', () => {
    ...
    expect(wrapper.find('.Pagination__page-button--current').text()).toBe('1');
  });
  ...
  it('render current page as max if current < max', () => {
    ...
    expect(wrapper.find('.Pagination__page-button--current').text()).toBe('10');
  });
});
...
describe('Arrows disabled state: ', () => {
  it('render arrows not disabled if 1 < current < max', () => {
    const wrapper = shallow(<Pagination max={10} current={5} onClick={jest.fn()} />);
    const arrows = wrapper.find('.Pagination__page-button--arrow');
  ...
  it('first disabled if current === 1', () => {
    const wrapper = shallow(<Pagination max={10} current={1} onClick={jest.fn()} />);
    expect(wrapper.find('.Pagination__page-button--arrow').first().prop('disabled')).toBe(true);
  });

  it('last disabled if current === max', () => {
    const wrapper = shallow(<Pagination max={10} current={10} onClick={jest.fn()} />);
    expect(wrapper.find('.Pagination__page-button--arrow').last().prop('disabled')).toBe(true);
  });
});
